...
my $readskfasta ='mir_K_had_adap_2200.fasta';
...
while (<READS_K_FASTA>) {
	...
	push @result_line,$readk_second_line,$readk_first_line_[1];
	chomp(@result_line);
...
sub doOperation
{
	...
	#open(RESULT_.$i, ">result_poz_".$i.".csv") or die "Can't write to results file: $!";
	my $RESULT_=GET_MIRNA_RESULTS_.$id;
